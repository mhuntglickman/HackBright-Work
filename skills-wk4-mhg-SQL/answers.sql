...
SELECT models.name, models.brand_name, brands.headquarters FROM models INNER JOIN brands ON models.brand_name = brands.name WHERE models.brand_name='Ford' AND models.name = 'Mustang';
...
SELECT name, year FROM models AS m WHERE m.year IN (SELECT founded FROM brands AS b WHERE b.founded=m.year);